...
#             if response.text != "accapted" and response.text != "accepted" and len(response.text) > 1:
#                 error_logger.error(
#                     '\t'.join(
...
def push_to_server(device_serial, user_id, log_time):
    ...
    try:
        response = request("POST", PUSH_URL, data=data)
        if response.status_code == 200:
            if response.text not in ["accapted", "accepted", "already accepted"]:
                error_logger.error(
                    '\t'.join(
                        ['Error during API Call.', str(user_id), str(log_time), str(device_serial), response.text]))
                logging.error(
...
def main():
    ...
    for index, device in enumerate(DEVICES):
        ip = device["ip"]
        print("Communicating with device {}".format(ip))
        keys = device.keys()
